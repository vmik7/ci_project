...
import 'pages/BuildDetails/BuildDetails.scss'

export interface BuildDetailsProps extends BasePageProps {}

const block = cn('build-details')

export const BuildDetails = memo<BuildDetailsProps>((props) => {
    ...
    }, [rebuildError, dispatch])

    useEffect(() => {
        if (isRebuilded && rebuildData) {
            navigate(`/build/${rebuildData.id}`)
            return () => {
                dispatch(nullRebuildData())
            }
        }
    }, [isRebuilded, rebuildData, dispatch, navigate])
